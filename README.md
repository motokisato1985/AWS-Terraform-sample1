...
- 2. `main.tf` 内の `backend "s3"` ブロックの `bucket` 名を、ご自身のバケット名に
　　　書き換えてください。
- 3. `provider.tf` 内の `profile` 指定が削除されていることを確認してください。
- 4. `example_terraform.tfvars` をコピーして `terraform.tfvars` を作成し、
　　　各変数をご自身の環境に合わせて設定してください。
...
　・NATゲートウェイのコスト削減のため、networkフォルダのvariables.tf内の`default` を `0` に変更　　し、terraform plan → terraform applyでリソースの削除を行ってください。
　・RDSはAWSマネジメントコンソール上で一時停止を行ってください。
　・ECSタスクはAWS CLIコマンドでタスクの停止を行ってください。
　(例) aws ecs update-service \
  --cluster nagoyameshi-dev-ecs-cluster \
  --service nagoyameshi-dev-ecs-service \
  --desired-count 0
